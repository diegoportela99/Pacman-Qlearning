...
Maze.prototype.draw = function () {
	...
	for (let i = 0; i < row_count; i++) {
		...
		for (let j = 0; j < row_length; j++) {
			...
			// in our pacman implementation a wall is 1 and aisles are 0, 2, 3, 4
			maze[i].push(pixel == 1 ? 1 : 0);
		}
	}

	const top = maze[1]
	const bottom = maze[maze.length - 2]
	const left = [];
	const right = [];

	for (let rowIndex in maze) {
		if (!(rowIndex === 0 || rowIndex === maze.length - 1)) {
			left.push(maze[rowIndex][1]);
			right.push(maze[rowIndex][maze.length - 2]);
		}
	}

	function getSidePathPixels(side, sideType) {
		const sidePathPixels = [];

		for (let sidePixelIndex in side) {
			if (side[sidePixelIndex] == 0) {
				switch (sideType) {
					case "top":
						sidePathPixels.push({x: sidePixelIndex, y: 0});
						break;
					case "bottom":
						sidePathPixels.push({x: sidePixelIndex, y: maze.length - 1});
						break;
					case "left":
						sidePathPixels.push({x: 0, y: sidePixelIndex});
						break;
					case "right":
						sidePathPixels.push({x: maze.length - 1, y: sidePixelIndex});
						break;
				}
			}
		}

		return sidePathPixels;
	}

	let topSelected = getSidePathPixels(top, "top");
	let bottomSelected = getSidePathPixels(bottom, "bottom");
	let leftSelected = getSidePathPixels(left, "left");
	let rightSelected = getSidePathPixels(right, "right");

	const sides = topSelected.concat(bottomSelected).concat(leftSelected).concat(rightSelected);
	// console.log("s", sides)

	const startPixel = sides[Math.floor(Math.random()*sides.length)];
	let endPixel = sides[Math.floor(Math.random()*sides.length)];

	while (startPixel.x === endPixel.x && startPixel.y === endPixel.y) {
		endPixel = sides[Math.floor(Math.random()*sides.length)];
	}

	maze[startPixel.y][startPixel.x] = 0;
	maze[endPixel.y][endPixel.x] = 0;
