...
class Ghost extends Character {
  ...
  isCloser(index, point) {
    ...
    return (
      Math.abs(nextX - pointX) > Math.abs(posX - pointX) ||
  ...
  makeIntelligentMove(point, toward) {
    ...
    while ((toward ? isCloser : !isCloser) && attempts) {
  ...
  hunt() {
    const pacManIndex = this.cells.findIndex((cell) =>
  ...
  // Reset the position for each ghost
  stop() {
    if (this.cells[this.position].className.includes("pacman")) {
      this.cells[this.position].className = this.removeClass("pacman");
      this.cells[490].className += " pacman";
    }
    this.cells[this.startPosition].className = this.className;
    this.cells[this.position].className = this.removeClass(this.className);
